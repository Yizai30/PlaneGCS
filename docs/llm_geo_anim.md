...
IntelligentGeometryAnimation->>GeometryGraph: Instantiate
GeometryGraph-->IntelligentGeometryAnimation: Instance
IntelligentGeometryAnimation->>LLMGeometryProcessor: Instantiate
LLMGeometryProcessor-->IntelligentGeometryAnimation: Instance
IntelligentGeometryAnimation->>GeometryConstraintEngine: Instantiate
GeometryConstraintEngine-->IntelligentGeometryAnimation: Instance

loop 解决方案表示有新的内容
Client->>IntelligentGeometryAnimation: NewContent

IntelligentGeometryAnimation->>LLMGeometryProcessor: updateGeometryGraph(NewContent)
LLMGeometryProcessor-->>IntelligentGeometryAnimation: Updated geometryGraph
IntelligentGeometryAnimation->>GeometryConstraintEngine: validateGeometry(geometryGraph)
GeometryConstraintEngine-->>IntelligentGeometryAnimation: True or False
alt is False
	IntelligentGeometryAnimation->>GeometryConstraintEngine: autoCorrectGeometry(geometryGraph)
	GeometryConstraintEngine-->>IntelligentGeometryAnimation: True or False
	IntelligentGeometryAnimation-->>Client: True or False
	alt is False
		Client->>Client:手动干预
	end
end
IntelligentGeometryAnimation->>LLMGeometryProcessor: parseAnimCommand(oldGeometryGraph, updatedGeometryGraph)
LLMGeometryProcessor-->>IntelligentGeometryAnimation: vector<AnimationCommand> animationCommands
IntelligentGeometryAnimation->>LLMGeometryProcessor: generateKeyframes(animationCommands)
end
```
